...
% The symbol $=$ is traditionally used for statements of identity or equality.
...
\ssubsection{Belonging}

As a second example, consider the symbol $\in$.
Let $a$ denote an object and let $A$ denote a set.
%Let us suppose that the object denoted by $a$ belongs to the set denoted by $A$ (see \sheetref{sets}{Sets}).
...
We read these symbols as \say{the object denoted by $a$ belongs to the set denoted by $A$}.\footnote{The symbol $\in$ is a stylized lower case Greek letter $\epsilon$, which is a mnemonic for the ancient Greek word $\grave{\epsilon} \sigma \tau \acute{\iota}$ which means, roughly, \say{belongs}.
