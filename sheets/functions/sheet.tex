...
A \t{function} $f$ (or \t{correspondence}, \t{mapping}, \t{map}) \t{from} a set $X$ \t{to} a set $Y$ is a relation (see \sheetref{relations}{Relations}) whose domain is $X$ and whose range is a subset of $Y$, such that for each $x \in X$, there exists a \textit{unique} $y \in Y$ so that $(x, y) \in f$.

We call the unique $y \in Y$ the \t{result} of the function \t{at} the \t{argument} $x$.
We call $Y$ the \t{codomain}.
If the range is $Y$ we say that $f$ is a function from $X$ \t{onto} $Y$ (or call $f$ \t{surjective}, \t{onto}).
...
%  <div data-littype='run'> ❲% ill-define.❳ </div>
%  <div data-littype='run'> ❲% —namely, the set of ordered pairs which that function produces—and leave the concept of function undefined.❳ </div>
%  <div data-littype='run'> ❲% They do not define functions in terms of sets.❳ </div>
%  
...
We abbreviate that the object denoted by $f$ is a function whose domain is a set $X$ and whose codomain is a set $Y$ by
    \[
f: X \to Y
    \]
...
A nonstandard notation is $X \to Y$, read as ``$A$ to $B$.''
