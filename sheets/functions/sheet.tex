...
A \t{function} (or \t{mapping} or \t{map}) $f$ \t{from} a set $X$ \t{to} a set $Y$ is a relation (see \sheetref{relations}{Relations}
\sheetref{relations}{Relations}) whose domain is $X$, whose range is a subset of $Y$, such that for each $x \in X$, there exists a unique $y \in Y$ so that $(x, y) \in f$.
...
$f$ is a mnemonic for function and $g$ and $h$ are nearby in the usual ordering of the Latin alphabet.
