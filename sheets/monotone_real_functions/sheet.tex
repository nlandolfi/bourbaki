...
A function is \t{monotone increasing} if $f(x) < f(y)$ whenever $x < y$, and \t{monotone nondecreasing} if $f(x) \leq f(y)$ whenever $x, y \in \R $ and $x < y$.\footnote{Unforunately, some authors use \say{monotone increasing}
for \say{monotone nondecreasing} and use the terminolgy \t{strictly monotone increasing} for \say{monotone increasing}.}
