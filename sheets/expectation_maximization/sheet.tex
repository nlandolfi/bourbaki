...
\ssection{Why}\footnote{Future editions will rework this sheet.}


\ssection{Definition}

Let $Z$ and $X$ be non-empty finite sets.  We want to model a distribution
$p^\theta: Z \times X \to \R$.
We parameterize a family of distributions by a parameter $\theta$.
We have a dataset $(x^1, \dots, x^n)$.
Given a parameter $\theta^0$, we want to solve
\[
...
\ssubsection{Binary Gaussian Mixture Example}\footnote{Future editions will expand.}
