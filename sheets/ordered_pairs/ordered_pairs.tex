...
Let $A$ and $B$ be non-empty \rt{sets}{set}.
...
The \t{cross product} of $A$ and $B$ is the set of all ordered pairs.
This set is also called the \t{cartesian product}.
If $A \neq B$, the ordering causes the cross product of $A$ and $B$ to differ from the cross product of
$B$ with $A$.
...
We denote the cartesian product of $A$ with $B$ by $A \cross B$, read aloud as \say{A cross B.}
