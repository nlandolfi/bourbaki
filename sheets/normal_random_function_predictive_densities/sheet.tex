...
Let $F: \Omega  \to (A \to \R )$ be a normal random function with mean function $m: A \to \R $ and covariance function $k: A \times A \to \R $ over the probability space $(\Omega , \mathcal{A} , \mathbfsf{P} )$.
Let the family of random variables (or stochastic process) of $F$ be $f: A \to (\Omega  \to \R )$.

Let $e$ be a normal random vector with mean zero and covariance $\Sigma _{e}$.
...
\begin{proposition}
Define $m_{a} \in \R ^{n}$ by $\transpose{(m(a^1),\cdots,m(a^n))}$ and define $m_{b}$ by $\transpose{(m(b^1), \cdots, m(b^m))}$.\footnote{Future editions will fix the re-use of the symbol $m$.}
Define $\Sigma _a \in \R ^{n \times n}$ by
  \[
\pmat{
