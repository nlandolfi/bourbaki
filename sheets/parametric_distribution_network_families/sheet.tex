%!name:parametric_distribution_network_families
%!need:directed_graph_distributions
%!need:parametric_distribution_families
...
In the case that $\pa_i = \emptyset$ in $G$, $\set{g_i^{(\theta)}}_\theta$ is a parametric distribution family on $A_i$ and in the case that $\pa_i \neq \emptyset$, $\set{g_i^{(\theta)}}_{\theta}$ is a parametric conditional distribution family on $A_i$ from $\prod_{j \in \pa_{i}} A_j$ (for both these terms, see \sheetref{parametric_distribution_families}{Parametric Distribution Families}).
