...
\begin{proposition}
$\complement{A \cup B} = \complement{A} \cap  \complement{B}$\end{proposition}
\begin{proposition}
$\complement{A \cap  B} = \complement{A} \cup \complement{B}$\end{proposition}
\subsection*{Principle of duality}

As a result of DeMorgan's Laws\footnote{Future editions will change the name to remove the reference to DeMorgan in accordance with the project's policy on naming.}
and basic facts about complements (see \sheetref{set_complements}{Set Complements}) theorems about sets often come in pairs.
