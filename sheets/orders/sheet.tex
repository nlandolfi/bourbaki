...
If I have three objects $a$, $b$, and $c$, and I arrange them in that \say{order} from left to right, I can always say whether one of the objects is \say{to the left of} another.
In contrast, we use the word \say{partial}  above to indicate that two elements need not be comparable.
For example, consider the equality relation on $\set{a, b, c}$.
This is really \say{no order} at all and $A$ is \say{totally unordered}, but it is (by our definition) a partial order.
...
\ssubsection{Example: set inclusion}
...
By \say{let $X$ be a partially ordered set} we always mean \say{let $X$ be the domain of a partial order}.
