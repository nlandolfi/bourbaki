...
Future editions will elaborate, perhaps with forward-looking connections to \sheetref{rectangular_functions}{Rectangular Functions}.
  }
...
The Greek letter $\chi $ is a mnemonic for \say{characteristic}.

The subscript indicates the set on which the function is one.
In other words, for all $B \subset A$, $\chi _{B}^{-1}(\set{1}) = B$.
  \ifhmode\unskip\fi\footnote{
Another notation used, when referring to these as \say{indicator functions,} is $1_{B}: A \to \set{0, 1}$ or $\mathbf{1}_B$.
  }
