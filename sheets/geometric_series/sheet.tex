...
It is believable that $1/2 , 1/4 , 1/8 , \dots $ has a convergent series.
And likewise with $1/3, 1/9, 1/27, \dots  $
...
\section*{Characterization of convergence}

Does the geometric series of $x$ converge?
In other words, does $(s_n)$ defined by $s_n = \sum_{k = 1}^{n} x^k$ have a limit.

For $x = 1$ and $x = -1$, we have seen (see \sheetref{real_series}{Real Series}) that the series diverges.
However for the cases $x = 1/2 $ and $x=1/3$ the geometric series converges.
...
\lim_{n \to \infty} \sum_{k = 1}^{n} x^k = \frac{x}{1-x}
  \]
If $\abs{x} \geq 1$ then the geometric series of $x$ diverges.\end{proposition}
\begin{proof}Define $s_n = \sum_{k = 1}^{n} x^k$.
Then
    \[
\begin{aligned}
