...
Then there exists a unique $A \in \R ^{m \times  n}$ satisfying $f(x) = Ax$ for all $x \in \R ^n$.\end{proposition}
\begin{proof}Evaluate $f$ at the standard unit vectors $e_i$.
The $i$th component of $e_i$ is 1 and all other components are 0.\end{proof}
Moreover, this matrix representation of $f$ is unique.

\begin{proposition}
If $A, B \in \R ^{m \times  n}$ are two matrices so that $f(x) = Ax = Bx$, then $A = B$.\end{proposition}
\begin{proof}We have $Ax - Bx = 0$ so $(A - B)x = 0$ for every $x$.
