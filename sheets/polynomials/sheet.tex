...
% <div data-littype='run'> The function $p: A → A$ is a ❬polynomial❭ of degree $d$
%    and order $d+1$ if there exists a $d+1$ length sequence
%    $(c_0, c_1, …, c_d)$ in $A$ so that
%    <div data-littype='displaymath'>
%     <div data-littype='run'> p(a) = c_0 + c_1a + ⋯ + c_da^d. </div>
%    </div></div>
%</div>
