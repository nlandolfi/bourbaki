...
The symbol $\in$ is a stylized lower case Greek letter $\epsilon$, which is a mnemonic for $\grave{\epsilon} \sigma \tau \acute{\iota}$ which means \say{belongs}.
In English, since $\epsilon$ is read aloud \say{ehp-sih-lawn,} $\in$ is a mnemonic for \say{element of}.
