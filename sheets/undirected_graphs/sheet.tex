...
An \t{undirected graph} (or \t{graph}, \t{network}) is a pair $(V, E)$ in which $V$ is a nonempty set and $E$ is a subset of \textit{unordered} pairs of elements in $V$.
We call the elements of $V$ the \t{vertices} (or \t{points}) of the graph and the elements of $E$ the \t{edges} (or \t{arcs}, \t{lines}).
...
Some authors call two \textit{edges} \t{adjacent} if they have exactly one common end vertex.

\subsection*{Self-loops}

Some authors include a concept of a self-loop, which is meant to be an edge from of a ``self-loop''---an edge from one node to itself.
However, we do not allow this as the set $\set{u, u}$ is \textit{not} a pair.
