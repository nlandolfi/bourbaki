...
A \t{memory chain} (\t{markov chain}\footnote{This term is universal. We avoid it in these sheets because of the Bourbaki project's policy on naming. The skeptical reader will note, at least, out term and this term terms have the same intials.}
, \t{memory model}, \t{markov model}) \t{on} a set $A$ of \t{length} $n$ is a joint distribution $p: A^n \to [0, 1]$ satisfying
    \[
...
The function $f$ is the first marginal $p_1$.\end{proposition}
