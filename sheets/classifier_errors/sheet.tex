...
We call the result of $u_i$ under classifier $G$ a \t{false positive} (or \t{type I error}, read \say{type one error}) if $v_i = -1$ and $G(u_i) = 1$ and a \t{false negative} (or \t{type II error}, read \say{type two error}) if $v_i = 1$ and $G(u_i) = -1$.
