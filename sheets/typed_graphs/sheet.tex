...
For $S \subset \set{1, \dots , n}$, we denote the \sheetref{direct_products}{product}$\prod_{s \in S} A_s$ by $A_S$.

If $G$ is \sheetref{directed_graphs}{directed}, we call a \sheetref{directed_graphs}{source vertex}\t{exogenous} and otherwise we call a vertex \t{endogenous}.
...
We call the ordered pair $(\bar{G}, f)$ a \t{function graph}\footnote{This sheet is not to be confused with the graph of a function (see \sheetref{functions}{Functions}).}
(or \t{function diagram}).
