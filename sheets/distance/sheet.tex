...
Our inspiration is the notion of distance in the plane (see \sheetref{plane_distance}{Plane Distance}) or in space (see \sheetref{space_distance}{Space Distance}).
...
This observation justifies the word \say{between.}
If it were not the case, then we should use different words, and be careful to speak of the distance \say{from} a first point \say{to} a second point.
...
In this language, the third observation says that if we know two objects are each half of a unit distance from a third object, then the two objects are close (their distance is less than a unit).
We might call this third object the reference object.
Here, then, is the usefulness of the third property: we can infer closeness of two objects if we know their distance to a reference object.
