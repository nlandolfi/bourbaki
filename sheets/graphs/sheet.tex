...
A \ct{graph}{graph} is a set and a relation on the set.
The graph is \ct{undirected}{undirectedgraph} if the relation is symmetric; otherwise the graph is \ct{directed}{directedgraph}.

A \ct{vertex}{vertex} of the graph is an element of the set.
The set is called the \ct{vertex set}{vertexset}.  An \ct{edge}{edge} of the graph is an element of the relation.
...
A path \ct{cycles}{cycles} if a vertex appears more than once.
A path is \ct{finite}{pathfinite} if the sequence is finite.
A \ct{loop}{loop} is a finite path that cycles once.
