...
Roughly, the corresponding statements are ``it is a five'', ``it is a four'', ``it is a heart'', and ``it is a face card''.
...
The \sheetref{set_equality}{principle of extension}says that this set is unique.
All our basic principles about sets (other than the \sheetref{set_equality}{principle of extension}) assert that we can construct new sets out of old ones in reasonable ways.
...
We assert that there is a set, denote it by $B$, for which belonging is equivalent to membership in $A$ and satisfaction of $s$.
In other words,
\[
...
With the principle of specification, a ``principle of a universal set'' would give a contradiction (called \t{Russell's paradox}).}
\begin{proof}Suppose there exists a set, denote it $A$ which contains all sets.
