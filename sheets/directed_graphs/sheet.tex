...
%!refs:bélla_bollobàs/modern_graph_theory/chapter_01/section_01
...
We call the elements of the first set \t{vertices} and the elements of the second \t{edges}.
...
The \t{indegree} of a vertex is number parents it has and the \t{outdegree} is the number of children it has; c.f. \sheetref{undirected_graphs}{Undirected Graphs}.
...
If $x$ is a vertex, and $(x,x)$ is an edge, we call such an edge a \t{self-loop}.
Many authors exclude self-loops in their definition of directed graphs, in order to make the situation correspond more closely to that of undirected graphs.
...
An oriented graph can be obtained from an undirected graph by selecting an \say{orientation} of the undirected edges.
