...
\begin{proposition}
Covariance is symmetric and billinear.\end{proposition}
\begin{proposition}
The covariance of a random variable with itself is its variance.\end{proposition}
\begin{proof}Let $f$ be a square-integrable real-valued random variable, then
  \[
\cov(f, f) = \E (ff) - \E (f)\E (f) = \E (f^2) - (\E (f))^2 = \var(f).
  \]\end{proof}
\begin{proposition}
The variance of a sum of integrable real-valued random variables whose pairwise products are integrable is the double sum of the pairwise covariances.\end{proposition}
