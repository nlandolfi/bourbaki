...
const SearchTemplateHTML = `<html>
	...
	<body>
		...
		<div>
			<form method="GET">
			<input style="border: 1px solid gray" value="{{ .Query }}" name="query"/>
			{{ len .Results }} results in {{ .SearchDuration | printf "%s" }}
			</form>
		</div>
			{{ range $r := .Results }}
			<div style="cursor:pointer; border: 1px solid gray; padding 5px; border-radius: 3px; padding: 12px; margin:3px" onclick="location.href='./sheets/{{ $r.Name}}.html'">
			<b> {{ title $r.Name }} </b>
